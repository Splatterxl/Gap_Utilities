const assert = (data, target) => data.toLowerCase() == target || data.toLowerCase().startsWith(target) || data.toLowerCase().includes(target),
  disc = async (ctx, target, type = "user") => {
    ...
    if (index == "cancel") { ctx.respond(ctx.util.embeds.neutralEmbed("Cancelled.")); return null; }
    ...
    if (!members.map(v => v)[index - 1]) { ctx.respond(ctx.util.embeds.errorEmbed("That user isn't in my list!"));
      m.delete(); return null; }
    m.delete()
