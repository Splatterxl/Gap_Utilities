...
module.exports.run = async (bot, msg, args, db, flags, ctx) => {
  switch (args[1]) {
      ...
      ctx.respond(
        new Discord.MessageEmbed({
          ...
          description: Object.entries(
            db.get("tags")[`g${msg.guild.id}`] ?? { ",,,none,,,": null }
          )
            .map((v) => (v[0] == ",,,none,,," ? "None" : `\`${v[0]}\``))
            .join(" | "),
          color: "YELLOW",
        })
      );
    ...
    case "create":
      if (!args[2])
        return ctx.respond(
          "<:redTick:796095862874308678> You need to specify a name!"
        );
      if (!args[3])
        return ctx.respond(
          "<:redTick:796095862874308678> You need to specify some text!"
        );
      if (
        db.get(`tags.g${msg.guild.id}.${args[2]}`) &&
        db.get(`tags.g${msg.guild.id}.${args[2]}`).author != msg.author.id
      )
        return ctx.respond(
          "<:redTick:796095862874308678> You don't own that tag!"
        );
      db.set(`tags.g${msg.guild.id}.${args[2]}`, {
    ...
    default:
      const tag = db.get(
        `tags.g${msg.guild.id}.${args[args[1] == "show" ? 2 : 1]}`
      );
      if (tag)
        ctx.respond(
          new Discord.MessageEmbed({
            color: "YELLOW",
            description: ctx.util.templang(tag.content, {
              args: ctx.args,
              author: {
                id:
                  (
                    await ctx.client.users
                      .fetch(tag.author, true)
                      .catch((e) => null)
                  )?.id ?? "0",
                tag:
                  (
        ...
        ctx.respond(
          "Invalid usage; should be `tag [list|create|show]` or `tag <name of tag>`"
        );
      break;
  }
};
