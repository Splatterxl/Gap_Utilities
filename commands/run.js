...
module.exports = {
    ...
    run: async (bot, msg, args) =>
    {
        ...
        {
            let now = Date.now();
            if (args.slice(1).join(' ') == 'process') return msg.reply(require('./mock').run(bot, msg, ['mock', 'oh', 'no,', 'dont', 'crash', 'me']));
            ...
            msg.reply(`\`\`\`js\n${proc.execSync(`cd tmp && cd ${now} && node index && rm -rf ${now}`).toString()}\`\`\``);
        } catch (e)
        {
            throw e;
        }
