...
module.exports = {
    ...
    run: async (bot, msg, args, db,  flags, ctx) =>
    {
        ...
        await target.ban({ reason: `[ Ban by ${msg.author.tag} ] ${args[2] ? args.slice(2) : "No reason specified."}`}).catch(r => { err = true; msg.react('❌'); return ctx.respond(new Discord.MessageEmbed({color: "RED", description:`<:redTick:796095862874308678> I couldn't ban that user.`})) });
        if (err) return;
