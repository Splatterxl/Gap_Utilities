 ...
 */
  ...
  switch(args[1]) {
  case "list":
  return ctx.respond(ctx.util.embeds.neutralEmbed(db.get(`reminders.${msg.author.id}`)?.map?.(v => `${moment(v.time).fromNow()} - ${v.content}`).join("\n") || "None", false));
  default:
  if (!args[2]) return ctx.respond(ctx.util.embeds.errorEmbed("Incorrect arguments! An argument for `reminder` was not provided."));
  ...
  ctx.respond(ctx.util.embeds.okEmbed(`I will remind you ${[...ctx.util.unixConvert(V.time)].map((v, i) => i == 0 ? v.toLowerCase() : v).join("")} (${moment(V.time).fromNow()})!`));
