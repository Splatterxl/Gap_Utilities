...
module.exports = {
  ...
  run: async (bot, msg, args, db, flags, ctx) => {
    const member = await ctx.util.get.member(ctx, args.slice(1).join(" "));
    ...
    const _ = new Discord.MessageEmbed({
      ...
      description: new Permissions(Permissions.ALL)
        .toArray()
        .map((perm) => `\`${perm.replace(/_/g, " ").toLowerCase().replace(/\b\w/g, v => v.toUpperCase())}\` | ${member.permissions.has(perm) ? '<:greenTick:796095828094615602>' : "<:redTick:796095862874308678>"}`)
        .join("\n"),
    });
    msg.channel.send(_);
  },
