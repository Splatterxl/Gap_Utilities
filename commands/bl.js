module.exports.run = async (a, b, args, d, e, ctx) => {
  ...
  usr = ((await cctx.client.users.fetch(target, true).catch(e => null)) ?? ctx.guild.members.cache.find(v => assert(v.user.id) || assert(displayName) || assert(v.user.tag) || assert(v.user.username))?.user);
