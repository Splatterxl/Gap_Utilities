...
module.exports = {
  run: async (bot, msg, args, db) => {
    ...
    db.ref(`warns/${msg.guild.id}/${idify(args[1])}`).set((async()=>{let warns = (await db.ref(`warns/${msg.guild.id}/${idify(args[1])}`).get()).val(); warns = warns ? warns : [];warns.push({moderator:msg.author, reason: args[2] ? args.slice(2).join(" ") : "No reason provided."}); return warns;})());
