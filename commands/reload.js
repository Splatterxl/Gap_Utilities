        ...
        case 'braincells':
          msg.reply(
            new Discord.MessageEmbed({
              ...
              description:
                "```require('./commandLoader.js')('braincells')\n\n^^^^^^^^\n\nNo module 'braincells' found.```",
            })
          );
          break;
        default:
          if (!flags.getObj().solo?.includes('event')) {
            if (
              ...
              try {
                let cmd = global.cmds.find(
                  c =>
                    c.help?.id == args[1] || c.help?.aliases?.includes(args[1])
                )?.help.id;
            ...
            if (
              ...
              try {
                ...
                global.cmds.set(
                  args[1],
                  require(`../events/${args[1]}.event`).prototype
                    ? new (require(`../events/${args[1]}.event`))()
                    : require(`../events/${args[1]}.event`)
                );
                msg.reply(
                  new Discord.MessageEmbed({
                    title: '<:greenTick:796095828094615602> Done!',
