...
if (perms.slice(0, cmd.help?.permLvl ?? 1).some(v => !v.every(v => msg.member.permissions.has(Discord.Permissions.FLAGS[v])))) {
  return ctx.respond(`You need to be a ${verbose[perms.indexOf(perms.find(v => !v.every(v => msg.member.permissions.has(Discord.Permissions.FLAGS[v]))))]} to use this command!`)
}
